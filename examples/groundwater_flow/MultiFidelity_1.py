...
def main():         
    ...
    if X_test_coarse1.shape == y_test.shape:
        logging.info(f"\nMSE coarse simulation 1:  {np.sqrt(np.mean((X_train_coarse1 - y_train)**2)):.4e}")
        logging.info(f"\nMSE coarse simulation 1:  {np.sqrt(np.mean((X_test_coarse1 - y_test)**2)):.4e}")
    ...
    print("Test Error: ", np.mean((mfnn_model.model([X_test_param,X_test_coarse1])-y_test)**2))
