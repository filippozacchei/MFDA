...
from pod_utils import reshape_to_pod_2d_system_snapshots, project, reshape_to_lstm, reconstruct, reconstruct_eff1

# Import necessary modules
from multi_fidelity_lstm import MultiFidelityLSTM

case = "MLDA2"  # Options: "2-step"/"1-step"/"FOM"
...
test_distribution = CustomUniform([0.01,0.5],[0.1,1.5])
for i in range(100):
    x_Test = test_distribution.rvs()
    print(x_Test)
print("Done")
...
for i in range(10):
    ...
    res = least_squares(ls,x_true, jac='3-point', verbose=2, bounds=([0.01,0.5],[0.1,1.5]))
    ...
    my_proposal = tda.GaussianRandomWalk(C=covariancep,scaling=1e-4, adaptive=True, gamma=1.1, period=10)
...
np.save(os.path.join(output_folder, f'MDA_MF_10_{case}.npy'), Time_ESS)
np.save(os.path.join(output_folder, f'MDA_MF_10_{case}.npy'), Times)
np.save(os.path.join(output_folder, f'MDA_MF_10_{case}.npy'), ERR)
np.save(os.path.join(output_folder, f'MDA_MF_10_{case}.npy'), ESS)
