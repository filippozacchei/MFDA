...
def train_model(config, X_train, X_train_coarse1, y_train, X_test, X_test_coarse1, y_test, Sigma):
    ...
    sfnn_model.train([X_train,X_train_coarse1], y_train, [X_test,X_test_coarse1], y_test)

    logging.info("Model training completed successfully!")



def evaluate_model(config, X_test, X_test_coarse1, U, Sigma, u_test_snapshots, scaler_Y):
...
def main():
    ...
    # Prepare datasets
    X_train, X_train_coarse1, y_train, X_test, X_test_coarse1, y_test, U, Sigma, u_test_snapshots, scaler_Y = load_and_process_data(config, num_modes=14)
