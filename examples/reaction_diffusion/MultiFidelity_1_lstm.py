...
def main():
    ...
    # Prepare datasets
    X_train, X_train_coarse1, y_train, X_test, X_test_coarse1, y_test, U, Sigma, u_test_snapshots, scaler_Y = load_and_process_data(config, num_modes=50)
    ...
    # Evaluate the model
    model = evaluate_model(config, X_test, X_test_coarse1, U, Sigma, u_test_snapshots, num_modes=50, scaler_Y=scaler_Y)
    
    predictions = model.predict((X_test, X_test_coarse1))/Sigma[:50]  # Shape: (batch_size, time_steps, num_modes)
    predictions_reshaped = predictions.reshape(-1, predictions.shape[-1])  # Shape: (batch_size * time_steps, num_modes)
    # predictions_inverse = scaler_Y.inverse_transform(predictions_reshaped)
    ...
    plot_2d_system_prediction(u_test_snapshots, train_data['x'], train_data['y'], n, n_steps=1001, save_path='./exact_mf1.gif')
    plot_2d_system_prediction(prediction, train_data['x'], train_data['y'], n, n_steps=1001, save_path='./predicted_mf1.gif')
    plot_2d_system_prediction(u_test_snapshots-prediction, train_data['x'], train_data['y'], n, n_steps=1001, save_path='./difference_mf1.gif')
