...
def load_and_process_data(config, num_modes=10):
    ...
    U, Sigma = pod_basis["POD_modes_coarse"], pod_basis["singular_values_fine"]
...
def train_model(config, X_train, y_train, X_test, y_test, Sigma):
    ...
    sfnn_model = SingleFidelityNN(
        ...
        output_units=y_train.shape[1],  # Ensure output is 3D
...
def evaluate_model(config, X_test, U, Sigma, u_test_snapshots, scaler_Y):
    ...
    reconstructed, error = reconstruct(U, Sigma, predictions, num_modes=40, original_snapshots=u_test_snapshots)
...
def main():
    ...
    config_filepath = 'config/config_SingleFidelity_nn.json'
    ...
    predictions = model.predict(X_test) # Shape: (batch_size, time_steps, num_modes)
    predictions = scaler_Y.inverse_transform(predictions)
    prediction = reconstruct(U,Sigma,predictions,num_modes=40)
    n = int(np.sqrt(prediction.shape[0]//2))
    nt=100
    plot_2d_system_prediction(u_test_snapshots[:,nt:], train_data['x'], train_data['y'], n, n_steps=1010, save_path='./gif/exact_sf_nn.gif')
    plot_2d_system_prediction(prediction[:,nt:], train_data['x'], train_data['y'], n, n_steps=1010, save_path='./gif/predicted_sf_nn.gif')
    plot_2d_system_prediction(np.log(np.abs(u_test_snapshots[:,nt:]-prediction[:,nt:])), train_data['x'], train_data['y'], n, n_steps=1010, save_path='./gif/difference_sf_nn.gif')
