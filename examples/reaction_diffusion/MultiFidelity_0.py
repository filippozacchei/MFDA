...
def evaluate_model(config, X_test, U, Sigma, u_test_snapshots, num_modes, scaler_Y):
    ...
    predictions = model.predict((X_test))/Sigma[:num_modes]  # Shape: (batch_size, time_steps, num_modes)
...
def main():
    ...
    # Evaluate the model
    model = evaluate_model(config, X_test, U, Sigma, u_test_snapshots, num_modes=40, scaler_Y=scaler_Y)
    ...
    plot_2d_system_prediction(u_test_snapshots, train_data['x'], train_data['y'], n, n_steps=1001, save_path='./gif/exact_sf.gif')
    plot_2d_system_prediction(prediction, train_data['x'], train_data['y'], n, n_steps=1001, save_path='./gif/predicted_sf.gif')
    plot_2d_system_prediction(u_test_snapshots-prediction, train_data['x'], train_data['y'], n, n_steps=1001, save_path='./gif/difference_sf.gif')
